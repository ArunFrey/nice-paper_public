# ---
# Supplementary Analysis: Ordinal treatment
# ---
...
# specify paths
tables_path <- "output/tables/supp/r_treat_ordinal/"
...
# generate ordinal treatment
allbus <- allbus %>%
    mutate(
        treat_ordinal = 
        ifelse(date > events$dates[events$places == "Nice"] & 
        date <= events$dates[events$places == "Würzburg"], "Nice",
        ifelse(date > events$dates[events$places == "Würzburg"] & 
        date <= events$dates[events$places == "Ansbach"], "Würzburg",
        ifelse(date > events$dates[events$places == "Ansbach"], "Ansbach", "Control"))),
        treat_ordinal = factor(treat_ordinal, levels = c("Control", "Nice", "Würzburg", "Ansbach")))
...
# Specify model
ivs_allbus <- paste0("~ treat_ordinal + ", paste(vars_allbus_z, collapse = " + "))
...
# specify model
ivs_refugee <- paste0("~ treat_ordinal + ", paste0(vars_refugee_z, collapse = " + "))
