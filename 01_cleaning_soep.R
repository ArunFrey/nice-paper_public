...
refugee <- refugee %>%
    mutate(
        ...
        stay_in_Germany = ifelse(bgpr68 == 1, 1,
            ifelse(bgpr68 == 2, 0, NA)
        ),

        # German language skills
        german_speaking_now = bgpr87,
        german_writing_now = bgpr88,
        german_reading_now = bgpr89,
        german_speaking_arrival = bgpr_l_91,
        german_writing_arrival = bgpr_l_92,
        german_reading_arrival = bgpr_l_93,

        # Hours per day studying German
        study_german = ifelse(bgpr199 == -2, 0, bgpr199),

        # Health and health satisfaction
        health_satis = bgpr298,
        health_stat = as.character(as_factor(bgpr299)),
        health_stat = ifelse(health_stat == -1, NA, health_stat),



        # Belonging with country of origin
        belonging_origin = ifelse(bgpr325 %in% c(1, 2), 1,
            ifelse(bgpr325 > 2, 0, NA)
        ),
