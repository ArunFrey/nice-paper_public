# ---
# Supplementary Analysis: excluding observations after Nice and before Würzburg
# ---
...
# specify paths
tables_path <- "output/tables/supp/r_treat_no_nice/"
plots_path <- "output/plots/supp/r_treat_no_nice/"
models_path <- "output/models/supp/r_treat_no_nice/"

# specify table suffix 
tab_suffix <- "attacks in Würzburg and Ansbach only"
lab_suffix <- "_no_nice"

# specify models
analysis_a <- allbus %>%
    filter(sample_35 == 1) %>%
    # exclude all observations after Nice and before Würzburg
    filter(!(date > events$dates[events$places == "Nice"] & date <= events$dates[events$places == "Würzburg"]))
