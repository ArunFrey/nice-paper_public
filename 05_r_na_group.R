# ---
# Supplementary Analysis: case-wise deletion by model group
# ---
...
# specify paths
tables_path <- "output/tables/supp/r_na_group/"
plots_path <- "output/plots/supp/r_na_group/"
models_path <- "output/models/supp/r_na_group/"

# specify table suffix 
tab_suffix <- "list-wise deletion of missing values by model group"
lab_suffix <- "_na_group"
...
# Run models - Casewise deletion by model group #####
...
fit_emo <- lapply(dvs_emo,
                  FUN = function(x) {
                    estimatr::lm_robust(formula(paste(x, ivs_allbus)),
                                        data = analysis_a[analysis_a$sample_emo==1, ],
                                        subset = sample_29 == 1, 
                                        se_type = "HC1"
                    )
                  }
...
fit_risk <- lapply(dvs_risk,
                   FUN = function(x) {
                     estimatr::lm_robust(formula(paste(x, ivs_allbus)),
                                         data = analysis_a[analysis_a$sample_risk==1, ],
                                         subset = sample_29 == 1,
                                         se_type = "HC1"
...
fit_dist <- lapply(dvs_dist,
                   FUN = function(x) {
                     estimatr::lm_robust(formula(paste(x, ivs_allbus)),
                                         data = analysis_a[analysis_a$sample_dist==1, ],
                                         subset = sample_29 == 1,
                                         se_type = "HC1"
...
fit_host <- lapply(dvs_host,
                   FUN = function(x) {
                     estimatr::lm_robust(formula(paste(x, ivs_refugee)),
                                         data = analysis_r[analysis_r$sample_host==1, ],
                                         subset = sample_29 == 1, 
                                         se_type = "HC1"
                     )
                   }
...
fit_mhealth <- lapply(dvs_mhealth,
                      FUN = function(x) {
                        estimatr::lm_robust(formula(paste(x, ivs_refugee)),
                                            data = analysis_r[analysis_r$sample_mhealth==1, ],
                                            subset = sample_29 == 1, 
                                            se_type = "HC1"
                        )
                      }
