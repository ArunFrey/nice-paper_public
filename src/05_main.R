...
# specify paths
tables_path <- "output/tables/main/"
...
# Same table (hostility - mental health)
texreg(c(fit_host[dvs_host], fit_mhealth[dvs_mhealth]),
       digits = 2,
       include.ci = FALSE,
       stars = c(0.01, 0.05, 0.1),
       booktabs = TRUE,
       custom.model.names = c("Discrimination", "Anti-immig. worries", "Feeling welcome", "Mental distress", "Mental health"),
       custom.coef.map = labs,
       fontsize = "small",
       caption.above = TRUE,
       use.packages = FALSE,
       caption = paste("Impact of July attacks on refugees' experience of hostility and wellbeing"),
       label = paste0("tab_host_mhealth", lab_suffix),
       center = TRUE,
       file = paste0(tables_path, "tab_host_mhealth.tex")
)
