# ---
# Supplementary Analysis: Weekly treatment effect
# ---
...
# specify paths
tables_path <- "output/tables/supp/r_treat_by_week/"
plots_path <- "output/plots/supp/r_treat_by_week/"
models_path <- "output/models/supp/r_treat_by_week/"

# specify table suffix 
tab_suffix <- "weekly analysis"
lab_suffix <- "_week"

# specify models
analysis_a <- allbus %>%
    filter(sample_35 == 1) %>%
    mutate(weeks_after = factor(ifelse(weeks_to_event < 0, 0,
        ifelse(weeks_to_event == 5, NA, weeks_to_event))
    ))
...
# Specify model
ivs_allbus <- paste0("~ weeks_after + ", paste(vars_allbus_z, collapse = " + "))
...
# specify model
ivs_refugee <- paste0("~ weeks_after + ", paste0(vars_refugee_z, collapse = " + "))
